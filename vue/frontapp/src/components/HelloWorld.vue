...
export default {
  ...
  created: async function() {
    try {
      const result = await axios.get("http://127.0.0.1:3333");
  ...
  methods: {
    taskCreate: async function() {
      try {
        const result = await axios.post("http://127.0.0.1:3333/task", {
    ...
    taskDelete: async function(id, index) {
      try {
        await axios.delete("http://127.0.0.1:3333/task/" + id);
        this.currentTask = "";
    ...
    taskUpdate: async function(id, val) {
      try {
        await axios.put("http://127.0.0.1:3333/task/" + id, {
          task: val
        });
        alert("タスクを修正しました");
        this.currentTask = "";
