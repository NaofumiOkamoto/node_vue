...
export default {
  ...
  created: async function() {
    try {
      const result = await axios.get("//192.168.241.12:3333");
  ...
  methods: {
    taskCreate: async function() {
      try {
        const result = await axios.post("//192.168.241.12:3333/task", {
    ...
    taskDelete: async function(id, index) {
      try {
        await axios.delete("//192.168.241.12:3333/task/" + id);
        this.currentTask = "";
    ...
    taskUpdate: async function(id, val) {
      try {
        await axios.put("//192.168.241.12:3333/task/" + id, {
          task: val
        });
        alert("タスクを修正しました");
        this.currentTask = "";
